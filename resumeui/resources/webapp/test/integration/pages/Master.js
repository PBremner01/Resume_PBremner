				...
				actions : {
							...
							success : function (aEntityData) {
								this.waitFor({
									...
									success : function (oList) {
										var sCurrentId,
											aItemsNotInTheList = aEntityData.filter(function (oObject) {
												return !oList.getItems().some(function (oListItem) {
													return oListItem.getBindingContext().getProperty("sub_area") === oObject.sub_area;
												});
											});

										if (!aItemsNotInTheList.length) {
											// Not enough items all of them are displayed so we take the last one
											sCurrentId = aEntityData[aEntityData.length - 1].sub_area;
										} else {
											sCurrentId = aItemsNotInTheList[0].sub_area;
										}
										...
										oCurrentItem.bindingPath = "/" + oList.getModel().createKey("Subject", {
											sub_area : sCurrentId
										});
					...
					iRememberTheListItem : function (oListItem) {
						...
						this.getContext().currentItem = {
							...
							id: oBindingContext.getProperty("sub_area"),
							title: oBindingContext.getProperty("sub_area")
						};
					}
				},
				...
				assertions : {
					...
					theListHeaderDisplaysZeroHits : function () {
						return this.waitFor({
							viewName : sViewName,
							id : "page",
							matchers : new PropertyStrictEquals({name : "title", value : "Subject (0)"}),
							success : function () {
								Opa5.assert.ok(true, "The list header displays 'Subject (0)'");
							},
					...
					theListShouldBeSortedAscendingOnName : function () {
						return this.theListShouldBeSortedAscendingOnField("sub_area");
					},
					...
					theListShouldHaveAllEntries : function () {
						...
						// retrieve all Subject to be able to check for the total amount
						...
						return this.waitFor({
							...
							success : function (oList) {
								...
								this.waitFor({
									id : "page",
									viewName : sViewName,
									matchers : new PropertyStrictEquals({name : "title", value : "Subject (" + iExpectedLength + ")"}),
									success : function () {
